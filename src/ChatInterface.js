...
import { ChevronRight, ChevronDown, MessageCircle,  Sparkles, ArrowRight, Utensils, Loader } from 'lucide-react';
...
const ChatInterface = ({ conversations, theme, openStory, setSelectedItemFromConversation, onClearChat, isLoading }) => {
  const shouldShowClearChat = conversations.length >= 3;
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto py-6">
      ...
      {shouldShowClearChat && (
        <IntegratedClearChatMessage onClearChat={onClearChat} theme={theme} />
      )}
    </div>
  );
};
